...
static void     ArrayIterator_setval(ArrayIteratorClass *this, ...)
{
    ...
    this->_array->_tab[this->_idx] = va_new(this->_array->_type, &va);
    va_end(va);
}
...
static void     Array_ctor(ArrayClass *this, va_list *args)
{
    ...
    if (!this || !args)
        raise("Can't create object.");
    size = va_arg(*args, size_t);
    type = va_arg(*args, Object *);
    this->_type = type;
    this->_tab = malloc(((Class *)type)->__size__ * size);
    this->_size = size;
    tmp = va_new(type, args);
    for (size_t i = 0; i < size; ++i) {
        this->_tab[i] = malloc(((Class *)type)->__size__);
        memcpy(this->_tab[i], tmp, ((Class *)type)->__size__);
    }
}
...
static void     Array_setitem(ArrayClass *this, ...)
{
    ...
    if (!this || !this->_tab)
        raise("Can't set item");
    va_start(va, this);
    index = va_arg(va, size_t);
    if (index >= this->_size)
        raise("Can't set item");
    delete(this->_tab[index]);
    this->_tab[index] = va_new(this->_type, &va);
    va_end(va);
}
