...
#include <iostream>
#include "ButtonReturnSettings.hpp"
#include "client/Scene/MainMenuScene.hpp"
#include "client/GameObjects/SoundClicked.hpp"

RType::Client::GameObjects::ButtonReturnSettings::ButtonReturnSettings(RType::ECS::ID id, RType::ECS::Position position, RType::ECS::SfmlText text, RType::Client::Client *client) noexcept
        : GameObject(std::move(id), std::move(position), std::move(text)), _currentButton(Target::MAIN_MENU), _client(client) {

}

void RType::Client::GameObjects::ButtonReturnSettings::update(const RType::AEvent &event, std::vector<std::shared_ptr<RType::ECS::IGameObject>> &, RType::ECS::IManager *scene) noexcept
{
