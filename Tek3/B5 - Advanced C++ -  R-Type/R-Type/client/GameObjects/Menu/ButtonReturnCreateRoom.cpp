...
#include <iostream>
#include "ButtonReturnCreateRoom.hpp"
#include "client/GameObjects/SoundClicked.hpp"
#include "client/Scene/NewGameScene.hpp"

RType::Client::GameObjects::ButtonReturnCreateRoom::ButtonReturnCreateRoom(RType::ECS::ID id, RType::ECS::Position position, RType::ECS::SfmlText text, RType::Client::Client *client) noexcept
        : GameObject(std::move(id), std::move(position), std::move(text)), _currentButton(Target::NEW_GAME), _client(client) {

}

void RType::Client::GameObjects::ButtonReturnCreateRoom::update(const RType::AEvent &event, std::vector<std::shared_ptr<RType::ECS::IGameObject>> &, RType::ECS::IManager *scene) noexcept
{
