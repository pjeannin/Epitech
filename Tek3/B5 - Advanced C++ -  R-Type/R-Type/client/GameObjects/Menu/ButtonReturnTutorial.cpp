...
#include <iostream>
#include "ButtonReturnTutorial.hpp"
#include "client/Scene/MainMenuScene.hpp"
#include "client/GameObjects/SoundClicked.hpp"

RType::Client::GameObjects::ButtonReturnTutorial::ButtonReturnTutorial(RType::ECS::ID id, RType::ECS::Position position, RType::ECS::SfmlText text, RType::Client::Client *client) noexcept
        : GameObject(std::move(id), std::move(position), std::move(text)), _currentButton(Target::MAIN_MENU), _client(client) {

}

void RType::Client::GameObjects::ButtonReturnTutorial::update(const RType::AEvent &event, std::vector<std::shared_ptr<RType::ECS::IGameObject>> &, RType::ECS::IManager *scene) noexcept
{
