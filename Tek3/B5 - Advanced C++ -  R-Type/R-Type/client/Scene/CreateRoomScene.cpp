...
#include "client/GameObjects/Menu/ButtonReturnCreateRoom.hpp"
#include "client/GameObjects/Menu/InputText.hpp"
#include "client/GameObjects/Text.hpp"
#include "CreateRoomScene.hpp"
#include "client/GameObjects/Menu/SpriteSelectionCreateRoom.hpp"
#include "client/GameObjects/Menu/ValidateCreateRoom.hpp"

void RType::Client::Scene::CreateRoomScene::addPlayer(std::string newPlayer)
{
    _players.push_back(newPlayer);
}

void RType::Client::Scene::CreateRoomScene::printPlayers(int startId)
{
    int nb = 1;
    int posX = 1240;
    int posY = 400;
...
RType::Client::Scene::CreateRoomScene::CreateRoomScene(std::shared_ptr<sf::RenderWindow> window, std::string myPseudo, RType::Client::Client *client) noexcept: AScene(std::move(window)), _client(client)
{
    ...
    _gameObjects.emplace_back(std::make_shared<RType::Client::GameObjects::SpriteSelectionCreateRoom>(RType::ECS::ID(6), RType::ECS::SfmlDrawable(_window, "../../assets/pictures/main menu/rectangleSelection.png", std::make_pair<unsigned int, unsigned int>(1448, 134), std::make_pair<unsigned int, unsigned int>(1448, 134), std::make_pair<unsigned int, unsigned int>(0, 0), std::make_pair<float, float>(0.5, 0.5)), RType::ECS::Position(0, 300), RType::ECS::SfmlSound("../../assets/sound/buttonHovered.ogg", _client->getEffectVolume()), _client));
    _gameObjects.emplace_back(std::make_shared<RType::Client::GameObjects::ValidateCreateRoom>(RType::ECS::ID(7), RType::ECS::SfmlDrawable(_window, "../../assets/pictures/validate.png", std::make_pair<unsigned int, unsigned int>(90, 90), std::make_pair<unsigned int, unsigned int>(90, 90), std::make_pair<unsigned int, unsigned int>(0, 0), std::make_pair<float, float>(0.65, 0.65)), RType::ECS::Position(475, 305)));
    ...
    _gameObjects.emplace_back(std::make_shared<RType::Client::GameObjects::ButtonReturnCreateRoom>(RType::ECS::ID(10), RType::ECS::Position(-10, 120), RType::ECS::SfmlText(_window, "l", _font, 50, sf::Color::White), _client));
    ...
    _gameObjects.emplace_back(std::make_shared<RType::Client::GameObjects::Text>(RType::ECS::ID(12), RType::ECS::Position(150, 300), RType::ECS::SfmlText(_window, "Ready", _font, 50, sf::Color::White)));
    _gameObjects.emplace_back(std::make_shared<RType::Client::GameObjects::Text>(RType::ECS::ID(13), RType::ECS::Position(150, 400), RType::ECS::SfmlText(_window, "Start the game", _font, 50, sf::Color::White), false));
    printPlayers(44);
}

void RType::Client::Scene::CreateRoomScene::update(const RType::AEvent &event) noexcept {
    AScene::update(event);
}
